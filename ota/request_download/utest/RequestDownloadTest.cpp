...
struct RequestDownloadTest : testing::Test{
    ...
    RequestDownloadTest(){
        ...
        v_loadProjectPath();
        std::string strMcuFilePath {std::string(PROJECT_PATH) + "/backend/mcu/mcu_data.txt"};
        std::string batteryFilePath {std::string(PROJECT_PATH) + "/backend/ecu_simulation/BatteryModule/battery_data.txt"};
        std::ofstream mcuFile {strMcuFilePath};
        std::ofstream batteryFile {batteryFilePath};

        // Write to file a map containing the OTA_UPDATE_STATUS_DID
        std::unordered_map<uint16_t, std::vector<uint8_t>> umapU16VecU8_DataMap {{OTA_UPDATE_STATUS_DID,{IDLE}}};
        FileManager::writeMapToFile(strMcuFilePath, umapU16VecU8_DataMap);
        FileManager::writeMapToFile(batteryFilePath, umapU16VecU8_DataMap);
        ...
        upRequestDownloadService = std::make_unique<RequestDownloadService>(socket, logger);
    ...
    std::unique_ptr<RequestDownloadService> upRequestDownloadService;
    int socket;
    int socket2;
    Logger logger;
};
...
TEST_F(RequestDownloadTest, MCUAuthenticationFailed){
    ...
    upRequestDownloadService->requestDownloadRequest(kMcuReceiverId, {});
...
TEST_F(RequestDownloadTest, ECUAuthenticationFailed){
    ...
    // Stored data is irrelevant at the moment, use an empty vector
    upRequestDownloadService->requestDownloadRequest(kBatteryEcuReceiverId, {});
    spCapturedFrame->capture();
    testFrames(expectedFrame,*spCapturedFrame);
...
TEST_F(RequestDownloadTest, IncorrectOtaState){
    ...
    upRequestDownloadService->requestDownloadRequest(kBatteryEcuReceiverId, kVecU8_storedData);
    spCapturedFrame->capture();
    testFrames(expectedFrame,*spCapturedFrame);
...
TEST_F(RequestDownloadTest, InvalidCompressionOrEncryption){
    ...
    upRequestDownloadService->requestDownloadRequest(kBatteryEcuReceiverId, kVecU8_invalidCompressionOrEncryptionStoredData);
...
TEST_F(RequestDownloadTest, InsufficientPayloadSize){
    ...
    upRequestDownloadService->requestDownloadRequest(kBatteryEcuReceiverId, kVecU8_insufficientPayloadStoredData);
    spCapturedFrame->capture(); // this is a frame sent by FileManager::setDIDValue
    spCapturedFrame->capture(); // this is the NRC frame
    testFrames(expectedFrame,*spCapturedFrame);
}
...
TEST_F(RequestDownloadTest, InvalidMemoryAddress){
    ...
    upRequestDownloadService->requestDownloadRequest(kBatteryEcuReceiverId, kVecU8_invalidMemoryAddressStoredData);
    spCapturedFrame->capture(); // this is a frame sent by FileManager::setDIDValue
    spCapturedFrame->capture(); // this is the NRC frame
    testFrames(expectedFrame,*spCapturedFrame);
...
TEST_F(RequestDownloadTest, InvalidMemorySize){
    ...
    upRequestDownloadService->requestDownloadRequest(kBatteryEcuReceiverId, kVecU8_invalidMemorySizeStoredData);
    spCapturedFrame->capture(); // this is a frame sent by FileManager::setDIDValue
    spCapturedFrame->capture(); // this is the NRC frame
    testFrames(expectedFrame,*spCapturedFrame);
...
TEST_F(RequestDownloadTest, DISABLED_NotLatestSoftwareVersion){
    ...
    upRequestDownloadService->requestDownloadRequest(kBatteryEcuReceiverId, kVecU8_notLatestSoftwareVersionStoredData);
...
TEST_F(RequestDownloadTest, Ready){
    ...
    // Use at least 7 elements in the stored data (see the storedData vector above)
    upRequestDownloadService->requestDownloadRequest(kMcuReceiverId, kVecU8_storedData);
